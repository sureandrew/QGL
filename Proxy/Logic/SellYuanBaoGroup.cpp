...
void CSellYuanBaoGroup::InitSellRecord(NetGroup* record)
{
	if (record)
	{
		...
		for (Index i = C_INDEX(0); i < record->GetCount(); ++i)
		{
			...
			if (pSellYB && find(m_recordList.begin(), m_recordList.end(), pSellYB) == m_recordList.end())
				m_recordList.push_back(pSellYB);
		}
		ReOrderRecordList();
	}
...
RPCResult CSellYuanBaoGroup::CB_RemoveSellYuanBao(LPCPEER pPeer, RPCContext &context, const UInt32 &sellYuanBaoID)
{
	for (Index i = C_INDEX(0); i < m_recordList.size(); ++i)
	{
		if (sellYuanBaoID == m_recordList[i]->GetSellYuanBao_id())
		{
			...
			if (gGlobal.g_pMenuSellYBProcess)
			{
				if (gGlobal.g_pMenuSellYBProcess->GetSelectedRecordID() == sellYuanBaoID)
					gGlobal.g_pMenuSellYBProcess->ResetSelectBuyYBT();
				gGlobal.g_pMenuSellYBProcess->UpdateSellYBRecord();
			}
...
RPCResult CSellYuanBaoGroup::CB_GetTempBank(LPCPEER pPeer, RPCContext &context, const UInt32 &YBT, const UInt32 &money)
{
	if (YBT == 0 && money == 0)
	{
		gGlobal.PrintMessage(gGlobal.GetStringTable()->Get(_T("MSG_SELL_YB_NO_TEMP_BANK")), CHANNELTYPE_SYSTEM);
		gGlobal.PrintPlayerMessage(gGlobal.GetStringTable()->Get(_T("MSG_SELL_YB_NO_TEMP_BANK")));
	}
	else
	{
		gGlobal.g_pMenuSellYBProcess->OpenTempBank(YBT, money);
	}
