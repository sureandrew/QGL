...
///////////////////////////////////////////////////////////
#pragma once
#ifndef _RPC_ITEMDATA_PROXY_H_
#define _RPC_ITEMDATA_PROXY_H_

#include "Common/RPCFunc.h"
#include "RPCEnum/ItemData_enum.h"

class ItemData_Proxy : public NetObject
{
...
public:
	INLINE ItemData_Proxy()
			{ _fields _m = {}; m = _m; }

};

INLINE RPCResult ItemData_Proxy::SendRequest(LPCPEER pPeer, RPCContext &context, const RPCMask &mask)
{
	if (pPeer == NULL) return RPC_RESULT_FAIL;
	Buffer* pBuf = SafeCreateObject(Buffer);
	Reuben::Network::RPCHeader header(RPC_ItemData_REQ_STATE);
	pBuf->Write<Reuben::Network::RPCHeader>(header);
	pBuf->Write<RPCContext>(context);
	pBuf->Write<RPCNetID>(m_netId);
	pBuf->Write<RPCMask>(mask);

	return RPCSend<2000172110>(pPeer, pBuf);
}


#ifdef _REUBEN_MATH_AES_H_
AES_DECRYPT_HEAD(2138537034)
	AES_IMIX_COLUMNS
	AES_ISHIFT_ROWS
	AES_ISUB_BYTES
	AES_IADD_ROUNDKEY
