...
///////////////////////////////////////////////////////////
#pragma once
#ifndef _RPC_PARTNERDATA_PROXY_H_
#define _RPC_PARTNERDATA_PROXY_H_

#include "Common/RPCFunc.h"
#include "RPCEnum/PartnerData_enum.h"

class PartnerData_Proxy : public NetObject
{
...
public:
	...
	virtual void OnUpdateHP(const UInt16 &HP, const UInt16 &maxHP);
	virtual void OnUpdateSP(const UInt16 &SP, const UInt16 &maxSP);
...
public:
	INLINE PartnerData_Proxy()
			{ _fields _m = {}; m = _m; }

};

INLINE RPCResult PartnerData_Proxy::SendRequest(LPCPEER pPeer, RPCContext &context, const RPCMask &mask)
{
	if (pPeer == NULL) return RPC_RESULT_FAIL;
	Buffer* pBuf = SafeCreateObject(Buffer);
	Reuben::Network::RPCHeader header(RPC_PartnerData_REQ_STATE);
	pBuf->Write<Reuben::Network::RPCHeader>(header);
	pBuf->Write<RPCContext>(context);
	pBuf->Write<RPCNetID>(m_netId);
	pBuf->Write<RPCMask>(mask);

	return RPCSend<1682840298>(pPeer, pBuf);
}


#ifdef _REUBEN_MATH_AES_H_
AES_DECRYPT_HEAD(1536901753)
	AES_IMIX_COLUMNS
	AES_ISUB_BYTES
	AES_IADD_ROUNDKEY
	AES_ISHIFT_ROWS
...
#endif //_RPC_PARTNERDATA_PROXY_H_
