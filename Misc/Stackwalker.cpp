...
#pragma warning(disable : 4267 4244 4312)

#undef _CrtIsValidHeapPointer
#define _CrtIsValidHeapPointer(x)	TRUE
...
typedef std::map<ULONG, AllocHashEntryType> AllocHashMap;

static AllocHashMap AllocHashTable;
...
//            Die synchronisation mu?eine Ebene höher erfolgen
...
static ULONG AllocHashDeinit(void) {
  ...
  for(AllocHashMap::iterator it = AllocHashTable.begin(); it != AllocHashTable.end(); ++it)
  {
    pHashEntry = &it->second;
    while(pHashEntry != NULL) {
      pHashEntryOld = pHashEntry;
      pHashEntry = pHashEntry->Next;
      if (pHashEntryOld != pHashEntry) {
...
//   pContext:   Context-Record, for retrieving Callstack (EIP and EBP is only needed)
//   nDataSize:  How many bytes
static void AllocHashInsert(void *pvData, long lRequestID, CONTEXT &Context, size_t nDataSize) {
  ULONG HashIdx;
  ...
  pHashEntry->pData = pvData;			// data pointer
    ...
    if (pHashEntry->lRequestID == lRequestID) {
      ...
      if (pHashEntryLast == NULL) {
        ...
        if (pHashEntry->Next == NULL) {
          ...
          //memset(pHashEntry, 0, sizeof(AllocHashEntryType));
		  AllocHashTable.erase(HashIdx);
        }
        else {
    ...
    if (pHashEntry->lRequestID != 0) {
      while(pHashEntry != NULL) {
        if ( (pHashEntry->cRemovedFlag <= 0) || (pHashEntry->cRemovedFlag > 1) ) {
          ulCount++;
          if (g_CallstackOutputType == ACOutput_XML)
            _ftprintf(fFile, _T("<LEAK requestID=\"%u\" pointer=\"0x%08x\" size=\"%u\">\n"), pHashEntry->lRequestID, pHashEntry->pData, pHashEntry->nDataSize);
          else
            _ftprintf(fFile, _T("RequestID: %12i, pointer: 0x%08x, Removed: %i, Size: %12i\n"), pHashEntry->lRequestID, pHashEntry->pData, pHashEntry->cRemovedFlag, pHashEntry->nDataSize);
          CONTEXT c;
          memset( &c, '\0', sizeof c );
          c.Eip = pHashEntry->dwEIPOffset;
          c.Ebp = pHashEntry->dwESPOffset;
...
void AllocHashOut(FILE *fFile) {
  ...
  for(AllocHashMap::iterator it = AllocHashTable.begin(); it != AllocHashTable.end(); ++it)
  {
    ...
    if (pHashEntry->lRequestID != 0) {
      while(pHashEntry != NULL) {
        if (g_CallstackOutputType == ACOutput_XML)
          _ftprintf(fFile, _T("<MEMUSED requestID=\"%u\" pointer=\"0x%08x\" size=\"%u\"\n"), pHashEntry->lRequestID, pHashEntry->pData, pHashEntry->nDataSize);
        else
          _ftprintf(fFile, _T("RequestID: %12i, pointer: 0x%08x, Removed: %i, Size: %12i\n"), pHashEntry->lRequestID, pHashEntry->pData, pHashEntry->cRemovedFlag, pHashEntry->nDataSize);
        pHashEntry = pHashEntry->Next;
      }
 ...
 x:   pop eax
  ...
  if (nAllocType != _HOOK_FREE) {
    if(lRequest != 0) // Always a valid RequestID should be provided (see comments in the header)
      AllocHashInsert(pvData, lRequest, c, nSize);
  }
...
static void EnumAndLoadModuleSymbols( HANDLE hProcess, DWORD pid, FILE *fLogFile )
{
  ...
  for ( it = modules.begin(); it != modules.end(); ++ it )
  {
    ...
    pSLM( hProcess, 0, const_cast<PSTR>(it->imageName.c_str()),
		const_cast<PSTR>(it->moduleName.c_str()), it->baseAddress, it->size );
  }
...
int InitAllocCheckWN(eAllocCheckOutput eOutput, LPCTSTR pszFileName, ULONG ulShowStackAtAlloc) {
  ...
  pfnOldCrtAllocHook = SetMemoryHook(MyAllocHook);
#endif
...
ULONG DeInitAllocCheck(void) {
  ...
  if (g_bInitialized) {
    ...
    SetMemoryHook(pfnOldCrtAllocHook);
...
// check file exist
static bool IsFileExist(PCHAR filename)
{
	DWORD attr;
  ...
  for ( frameNum = 0; ; ++ frameNum )
  {
    ...
    else
    {
      ...
      { // yes, we have SymGetLineFromAddr()
        ...
        {
			// skip entry if no source file
			if (Line.FileName[0] == '\0' || !IsFileExist(Line.FileName))
				continue;
