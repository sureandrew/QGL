...
void PathFinder::CalcGuildLink()
{
	...
	*/
	...
	StlMap<UInt16, bool, std::less<UInt16>, ManagedAllocator<UInt16, bool> > > closeList;
	FDistList distList;
	StlMap<UInt16, UInt16, std::less<UInt16>, ManagedAllocator<UInt16, UInt16> > > prevList;
							  ...
							  PointList& ptlist)
{
	...
	// source position to first guide point
	DrawLine(x1, y1, m_guidePoints[minGuide1].PosX, m_guidePoints[minGuide1].PosY, points, d);
	ptlist.insert(ptlist.end(), points.begin(), points.end());

	// path between guide points
	UInt16 prevGuide;
	while (minGuide1 != minGuide2)
	{
		prevGuide = minGuide1;
		minGuide1 = m_linkGraph.Get(minGuide1, minGuide2);
		if (prevGuide < minGuide1) {
			DrawLine(m_guidePoints[prevGuide].PosX, m_guidePoints[prevGuide].PosY, 
				m_guidePoints[minGuide1].PosX, m_guidePoints[minGuide1].PosY, points, d);
		} else {
			DrawLine(m_guidePoints[minGuide1].PosX, m_guidePoints[minGuide1].PosY, 
				m_guidePoints[prevGuide].PosX, m_guidePoints[prevGuide].PosY, points, d);
			points.pop_back();
			points.insert(points.begin(), m_guidePoints[minGuide1]);
			reverse(points.begin(), points.end());
		}
		ptlist.insert(ptlist.end(), points.begin(), points.end());
	}
	// last guide point to destination position
	DrawLine(m_guidePoints[minGuide1].PosX, m_guidePoints[minGuide1].PosY, x2, y2, points, d);
	ptlist.insert(ptlist.end(), points.begin(), points.end());
