...
int _tmain(int argc, TCHAR* argv[])
{
	...
	if (gConfigure.Initialize(argv[1], argv[2]))
	{
		...
		InitLogger();
		DEBUG_INFODTL_1(GLOBAL_LOGGER, _F("memory status after init, %s"), MemoryManager::GetInstance().GetStatus().c_str());
		...
		MapApplication::GetInstance().Run();
		MapApplication::DestroyInstance();
		LocalRPCManager::GetInstance().DestroyInstance();
	}

	TRACE_ALWAYSDTL_1(GLOBAL_LOGGER, _F("Service %s Shutdown"), GLOBAL_NAME);
	DEBUG_INFODTL_1(GLOBAL_LOGGER, _F("memory status after main loop, %s"), MemoryManager::GetInstance().GetStatus().c_str());

	gConfigure.Uninitialize();
	DEBUG_INFODTL_1(GLOBAL_LOGGER, _F("memory status after config uninit, %s"), MemoryManager::GetInstance().GetStatus().c_str());
