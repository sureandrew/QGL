...
void CAnimTimer::Initialize()
{
	...
	m_hurtMoving = config.GetInt(_T("Setting"), _T("hurtMoving"), 250);
	...
	// pre-calculate all item animation time
	SetItemDelay();
...
Boolean CAnimTimer::LoadCSV(const TCHAR *filename)
{
	...
	if (reader.Open(fullname.c_str(), true))
	{
		...
		{
			TRACE_ERRORDTL_1(GLOBAL_LOGGER, _FE("invalid column format, filename %s"), filename);
			reader.Close();
...
void CAnimTimer::SetItemDelay()
{
	...
	gGlobal.m_resource->GetItemList(itemList);		// get all skill data

	for (ItemDataList::iterator it = itemList.begin(); it != itemList.end(); ++it)
	{
		pItem = *it;
		// skip not displayable skill
		if ((pItem->effect_Atk[0] == _T('\0') || pItem->effect_Atk[0] == _T('0')) && 
			(pItem->effect_Hurt[0] == _T('\0') || pItem->effect_Hurt[0] == _T('0')) &&
			(pItem->effect_Spark[0] == _T('\0') || pItem->effect_Spark[0] == _T('0')))
			continue;
		...
		if (pItem->effect_Atk[0] != _T('\0') && pItem->effect_Atk[0] != _T('0'))
		{
			_tcscpy(entName, pItem->effect_Atk);
			it2 = m_entStateMap.find(_tcslwr(entName));
			if (it2 != m_entStateMap.end())
			{
		...
		// get skill hurt time
		if (pItem->effect_Hurt[0] != _T('\0') && pItem->effect_Hurt[0] != _T('0'))
		{
			_tcscpy(entName, pItem->effect_Hurt);
			it2 = m_entStateMap.find(_tcslwr(entName));
			if (it2 != m_entStateMap.end())
			{
		...
		if (pItem->effect_Spark[0] != _T('\0') && pItem->effect_Spark[0] != _T('0'))
		{
			_tcscpy(entName, pItem->effect_Spark);
			it2 = m_entStateMap.find(_tcslwr(entName));
			if (it2 != m_entStateMap.end())
			{
		...
		if (sa.reactTime > 0)
			m_itemMap.insert(std::make_pair(pItem->item_id, sa));
	}
}
...
UInt32 CAnimTimer::GetActionTime(CBatCharacter *pSrcChar, CBatMob *pSrcMob, CBatActor * pTarget, UInt8 action,
								 UInt16 subId, UInt16 multi)
{
	...
	if (pTarget != NULL)
	{
		if (pTarget->IsUpdated(CBatActor::HPMask) &&
			pTarget->GetDShowHP())
			numShowCount ++;

		if (pTarget->IsUpdated(CBatActor::SPMask) &&
			pTarget->GetDShowSP())
			numShowCount ++;

		if (pTarget->IsUpdated(CBatActor::DPMask) &&
			pTarget->GetDShowDP())
			numShowCount ++;

		if (pTarget->IsUpdated(CBatActor::WCMask) &&
			pTarget->GetDShowWC())
			numShowCount ++;

		if (pTarget->IsUpdated(CBatActor::VPMask) &&
			pTarget->GetDShowVP())
			numShowCount ++;
	}
	UInt32 totalTime = 0;
	...
	case BATCMD_SKILL:
		{
			...
			if (pSrcChar) {
				animType = (sa.motionType == 1 ? ANIM_W1ATK1_S1 : 
			(sa.motionType == 2 ? ANIM_W2ATK1_S1 : ANIM_SKILL));

			if (animType == ANIM_SKILL)
			{
				BodyActionMap::iterator it = m_charMap.find(
					BodyAction(pSrcChar->GetSexClass(), animType));
				if (it != m_charMap.end())
				...
				animType = (sa.motionType != 0 ? ANIM_W1ATK1_S1 : ANIM_SKILL);
			...
			if (sa.animType == 1 || sa.animType == 4)				// motion time before casting skill
				totalTime += m_actMoving / 2;
			else if (sa.animType == 2 || sa.animType == 5)
				totalTime += m_actMoving;
	...
	case BATCMD_ITEM:
		{
			ItemActionMap::iterator it = m_itemMap.find(subId);
			if (it != m_itemMap.end())
			{
				ItemAction &sa = it->second;
				totalTime = sa.reactTime;
			}
	...
	//if (totalTime > 0)
	//	TRACE_VERBOSEL_5(GLOBAL_LOGGER, _F("time %d, actorId %d, action %d, subId %d, multi %d"), 
	//	totalTime, pSrcChar ? pSrcChar->GetChar_id() : pSrcMob->GetMob_id(), action, subId,
	//	multi);

	if (numShowCount > 1)
	{
		totalTime += NUM_DISPLAY_TIME * (numShowCount - 1);
	}
	if (pTarget && pTarget->GetDShowHP() < 0)
	{
		totalTime += m_hurtMoving;
	}
	return totalTime;
}
...
void CAnimTimer::DebugInfo()
{
	...
	TRACE_INFOL_4(GLOBAL_LOGGER, _F("actMoving: %d, escapeMoving %d, escaped %d, catchMoving %d"),
		m_actMoving, m_escapeMoving, m_escaped, m_catchMoving);

	TRACE_INFOL(GLOBAL_LOGGER, _F("charMap:"));
	for (BodyActionMap::iterator it = m_charMap.begin(); it != m_charMap.end(); ++it)
	{
		const BodyAction &ba = it->first;
		TRACE_INFOL_3(GLOBAL_LOGGER, _F("sexClass %d: action %d, time %d"), ba.sexClass,
			ba.action, it->second);
	}

	TRACE_INFOL(GLOBAL_LOGGER, _F("mobMap:"));
	for (BodyActionMap::iterator it = m_mobMap.begin(); it != m_mobMap.end(); ++it)
	{
		const BodyAction &ba = it->first;
		TRACE_INFOL_3(GLOBAL_LOGGER, _F("mob_id %d: action %d, time %d"), ba.mob_id,
			ba.action, it->second);
	}

	TRACE_INFOL(GLOBAL_LOGGER, _F("weaponMap:"));
	for (WeaponTypeMap::iterator it = m_weaponMap.begin(); it != m_weaponMap.end(); ++it)
	{
		const WeaponType &wt = it->first;
		TRACE_INFOL_3(GLOBAL_LOGGER, _F("item_id %d: sexClass %d, weaponType %d"), wt.item_id,
			wt.sexClass, it->second);
	}

	TRACE_INFOL(GLOBAL_LOGGER, _F("skillMap:"));
	for (SkillActionMap::iterator it = m_skillMap.begin(); it != m_skillMap.end(); ++it)
	{
		const SkillAction &sa = it->second;
		TRACE_INFOL_4(GLOBAL_LOGGER, _F("skill_id %d: animType %d, motionType %d, time %d"), 
