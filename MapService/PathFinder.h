...
public:
	...
	inline SetBlockingActive(Boolean value) { m_blockActive = value; }
...
inline bool PathFinder::IsEmpty(UInt16 x, UInt16 y)
{
	...
	return ((m_map[m_width * y + x] & TILE_GUIDE) == TILE_NORMAL);
}
...
struct PathFindData
{
	...
	PathFinder::GuideList m_gList;
	UInt16 Index;
	StlVector<BlockPos> PreSteps;
	PathFindData() : Index(0) {}
	...
	VOID Init()
	{
		...
		if (m_ptList.size() > 0)
		{
			BlockPos TempPos = m_ptList[0];
			m_ptList.clear();
			m_ptList.push_back(TempPos);
		}
		else
			m_ptList.clear();
