...
//-- Payne Add Begin (2006-Dec-21)

#if defined(_MSC_VER) && (_MSC_VER > 1100) || (__BORLANDC__)
#	define STDSTRING_EXPORT __declspec(dllexport)
#	define STDSTRING_IMPORT __declspec(dllimport)
#	define STDSTRING_C_EXPORT extern "C" __declspec(dllexport)
#	define STDSTRING_C_IMPORT extern "C" __declspec(dllimport)
#else
#	define STDSTRING_EXPORT
#	define STDSTRING_IMPORT
#	define STDSTRING_C_EXPORT
#	define STDSTRING_C_IMPORT
#endif

#if !defined(STDSTRING_LIB) && !defined(STDSTRING_DLL)
#define STDSTRING_LIB
#endif

#if defined(STDSTRING_LIB)
#	define STDSTRING_API
#	define STDSTRING_C_API
#	define STDSTRING_TEMPLATE
#elif defined(STDSTRING_DLL)
#	if defined(STDSTRING_EXPORTS)
#		define STDSTRING_API STDSTRING_EXPORT
#		define STDSTRING_C_API STDSTRING_C_EXPORT
#		define STDSTRING_TEMPLATE
#	else
#		define STDSTRING_API STDSTRING_IMPORT
#		define STDSTRING_C_API STDSTRING_C_IMPORT
#		define STDSTRING_TEMPLATE extern
#	endif
#endif

//-- Payne Add End (2006-Dec-21)
...
// Turn off unavoidable compiler warnings
	...
	#pragma warning (disable : 4311) // pointer truncation number type warning 
#endif
...
// -----------------------------------------------------------------------------
#define SS_STRINGA		std::basic_string<char, std::char_traits<char>, CAlloc>
#define SS_STRINGW		std::basic_string<wchar_t, std::char_traits<wchar_t>, CAlloc>
#define SS_SIZETYPE		SS_STRINGA::size_type	 // just for shorthand, really
#define SS_PTRTYPE		SS_STRINGA::pointer	  
#define SW_SIZETYPE		SS_STRINGW::size_type
#define SW_PTRTYPE		SS_STRINGW::pointer

template<typename CAlloc>
inline void	ssasn(SS_STRINGA & sDst, const SS_STRINGA & sSrc)
{
...
template<typename CAlloc>
inline void	ssasn(SS_STRINGA & sDst, PCSTR pA)
{
...
template<typename CAlloc>
inline void	ssasn(SS_STRINGA & sDst, const SS_STRINGW & sSrc)
{
	if ( sSrc.empty() )
	{
		sDst.erase();
...
template<typename CAlloc>
inline void	ssasn(SS_STRINGA & sDst, PCWSTR pW)
{
	int nSrc	= sslen(pW);
	if ( nSrc > 0 )
	{
...
template<typename CAlloc>
inline void ssasn(SS_STRINGA & sDst, const int nNull)
{
	UNUSED(nNull);
	ASSERT(nNull==0);
	sDst.assign("");
...
template<typename CAlloc>
inline void	ssasn(SS_STRINGW & sDst, const SS_STRINGW & sSrc)
{
	if ( sDst.c_str() != sSrc.c_str() )
	{
		sDst.erase();
...
template<typename CAlloc>
inline void	ssasn(SS_STRINGW & sDst, PCWSTR pW)
{
	// Watch out for NULLs, as always.
...
template<typename CAlloc>
inline void	ssasn(SS_STRINGW & sDst, const SS_STRINGA & sSrc)
{
	if ( sSrc.empty() )
	{
		sDst.erase();
...
template<typename CAlloc>
inline void	ssasn(SS_STRINGW & sDst, PCSTR pA)
{
	int nSrc	= sslen(pA);
...
template<typename CAlloc>
inline void ssasn(SS_STRINGW & sDst, const int nNull)
{
	UNUSED(nNull);
	ASSERT(nNull==0);
	sDst.assign(L"");
...
template<typename CAlloc>
inline void	ssadd(SS_STRINGA & sDst, const SS_STRINGW & sSrc)
{
	int nSrc	= static_cast<int>(sSrc.size());
...
template<typename CAlloc>
inline void	ssadd(SS_STRINGA & sDst, const SS_STRINGA & sSrc)
{
	sDst += sSrc;
}

template<typename CAlloc>
inline void	ssadd(SS_STRINGA & sDst, PCWSTR pW)
{
	int nSrc		= sslen(pW);
	if ( nSrc > 0 )
	{
...
template<typename CAlloc>
inline void	ssadd(SS_STRINGA & sDst, PCSTR pA)
{
	if ( pA )
	{
		...
		if ( pA >= sDst.c_str() && pA <= sDst.c_str()+sDst.length())
		{
			...
			{
				sDst.resize(sDst.size()+sslen(pA));
				sDst.append(pA);
			}
			else
				sDst.append(pA);
...
template<typename CAlloc>
inline void	ssadd(SS_STRINGW & sDst, const SS_STRINGW & sSrc)
{
	sDst += sSrc;
}

template<typename CAlloc>
inline void	ssadd(SS_STRINGW & sDst, const SS_STRINGA & sSrc)
{
	if ( !sSrc.empty() )
	{
		int nSrc	= static_cast<int>(sSrc.size());
...
template<typename CAlloc>
inline void	ssadd(SS_STRINGW & sDst, PCSTR pA)
{
	int nSrc		= sslen(pA);
...
template<typename CAlloc>
inline void	ssadd(SS_STRINGW & sDst, PCWSTR pW)
{
	if ( pW )
	{
		// If the string being added is our internal string or a part of our
		...
		if ( pW >= sDst.c_str() && pW <= sDst.c_str()+sDst.length())
		{
			if ( sDst.capacity() <= sDst.size()+sslen(pW) )
				sDst.append(std::wstring(pW).c_str());
			else
				sDst.append(pW);
		}
		else
...
template<typename CT, typename CAlloc>
class STDSTRING_API CStdStr : public std::basic_string<CT, std::char_traits<CT>, CAlloc>
{
	// Typedefs for shorter names.  Using these names also appears to help
	// us avoid some ambiguities that otherwise arise on some platforms

	#define MYBASE std::basic_string<CT, std::char_traits<CT>, CAlloc>	 // my base class
	//typedef typename std::basic_string<CT>		MYBASE;	 // my base class
	typedef CStdStr<CT, CAlloc>					MYTYPE;	 // myself
...
public:
	...
	CStdStr(const SS_STRINGA & str)
	{
		ssasn<CAlloc>(*this, SSREF(str));
	}

	CStdStr(const SS_STRINGW & str)
	{
		ssasn<CAlloc>(*this, SSREF(str));
	}
	...
	{
		ssasn<CAlloc>(*this, pA);
		return *this;
	}

	MYTYPE& operator=(PCWSTR pW)
	{
		ssasn<CAlloc>(*this, pW);
		return *this;
	}
	...
	{
		ssasn<CAlloc>(*this, reinterpret_cast<PCSTR>(pU));
		return *this;
	}
#endif
	...
	MYTYPE& operator+=(PCSTR pA)
	{
		ssadd<CAlloc>(*this, pA);
		return *this;
	}

	MYTYPE& operator+=(PCWSTR pW)
	{
		ssadd<CAlloc>(*this, pW);
		return *this;
	}
...
//		}
	...
	bool Load(UINT nId, HMODULE hModule=NULL)
	{
		...
		if ( NULL == hrsrc )
		{
			TRACE(_T("Cannot find resource %d: 0x%p"), nId, ::GetLastError());
		}
		else if ( 0 == (dwSize = ::SizeofResource(hModule, hrsrc) / sizeof(CT)))
		{
			TRACE(_T("Cant get size of resource %d 0x%p\n"),nId,GetLastError());
		}
		else
		{
		...
		if ( !bLoaded )
			TRACE(_T("String not loaded 0x%p\n"), ::GetLastError());
...
//	void			UnlockBuffer(); { }	// why have UnlockBuffer w/o LockBuffer?
	...
	HRESULT StreamSave(IStream* pStream) const
	{
		...
		{
			TRACE(_T("StreamSave: Cannot write control header, ERR=0x%p\n"),hr);
		}
		else if ( empty() )
		{
			;		// nothing to write
		...
		{
			TRACE(_T("StreamSave: Cannot write string to stream 0x%p\n"), hr);
		}
	...
	HRESULT StreamLoad(IStream* pStream)
	{
		...
		{
			TRACE(_T("StreamLoad: Cant read control header, ERR=0x%p\n"), hr);
		}
			...
			if ( (hdr.byCtrl & SSSO_UNICODE) != 0 )
			{
				...
				if ( sizeof(CT) == sizeof(wchar_t) )
				{
					if ( FAILED(hr=pStream->Read(pMyBuf, nBytes, &nRead)) )
						TRACE(_T("StreamLoad: Cannot read string: 0x%p\n"), hr);
				}
				else
				{	
...
typedef CStdStr<char, std::allocator<char> >		CStdStringA;	// a better std::string
typedef CStdStr<wchar_t, std::allocator<wchar_t> >	CStdStringW;	// a better std::wstring
typedef CStdStr<OLECHAR, std::allocator<OLECHAR> >	CStdStringO;	// almost always CStdStringW
...
{
	return s1 + CStdStringA(s2.c_str());
}
...
inline CStdStringA operator+(const CStdStringA& s1, PCWSTR pW)
{
	...
	{
		return CStdStringW(pW) + CStdStringW(sA.c_str());
	}
	...
	{
		return pA + CStdStringA(sW.c_str());
	}
#endif
...
inline CStdStringW operator+(PCWSTR pW, const CStdStringW& sW)
{
	...
	CStdStringW::size_type nLitSize = 
...
{
	return s1 + CStdStringW(s2.c_str());
}
...
inline CStdStringW WUFormatW(PCWSTR szwFormat, ...)
{
	...
	inline CStdStringA WUFormatA(UINT nId, ...)
	{
		...
		CStdStringA strOut;
		if ( strFmt.Load(nId) )
			strOut.FormatV(strFmt.c_str(), argList);
	...
	inline CStdStringA WUSysMessageA(DWORD dwError, DWORD dwLangId=SS_DEFLANGID)
	{
		...
		if ( 0 != ::FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM, NULL, dwError,
			...
			return WUFormatA("%s (0x%p)", szBuf, dwError);
		else
 			return WUFormatA("Unknown error (0x%p)", dwError);
	}
	inline CStdStringW WUSysMessageW(DWORD dwError, DWORD dwLangId=SS_DEFLANGID)
	{
		...
		if ( 0 != ::FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM, NULL, dwError,
								   dwLangId, szBuf, 511, NULL) )
			return WUFormatW(L"%s (0x%p)", szBuf, dwError);
		else
 			return WUFormatW(L"Unknown error (0x%p)", dwError);
	}
#endif
