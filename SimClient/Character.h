...
public:
	...
	virtual RPCResult CB_AntiHack(LPCPEER pPeer, RPCContext &context, const ByteArray &image, const AntiHackData &answerList);
	...
	virtual RPCResult CB_LearnLineup(LPCPEER pPeer, RPCContext &context, const UInt16 &uiLineupId);
	...
	virtual RPCResult CCharacter::CB_UpdateGenerationAcc(LPCPEER pPeer, RPCContext &context, const UInt32 &uiCurAcc, const UInt32 &uiHisAcc);
	...
	virtual RPCResult CB_BuyPointsShopItem(LPCPEER pPeer, RPCContext &context, const int &iRetCode, const UInt32 &uiPoints){return RPC_RESULT_OK;}
	...
	Boolean HasParty() { return GetHasParty(); }
	Boolean IsLeader() { return (m_pParty == NULL || GetIsLeader()); }
	Boolean IsInBattle();
	void Move(LPCPEER pPeer, RPCContext &context, UInt16 x, UInt16 y);
	void DisplayHackQuestion(Boolean server, UInt8 direc, UInt8 guy);
	int GetValidItem();
	void GivenProcess(RPCNetID &targetId, UInt8Vector &bagIndices, UInt8Vector &stackList, 
					UInt32Vector &partnerIdList, UInt32Vector &petIdList, UInt32 money);
	virtual String DebugInfo();

	UInt8 m_ClntHackPerson;
	UInt8 m_ClntHackDirec;
	UInt8 m_WrongAns;
	Boolean m_bServHack;
	Boolean m_bHackInProcess;
	bool m_bIsBuyingChargeItem;
};
