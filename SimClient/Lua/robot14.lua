...
function MapAction(map, character)
	...
	local hasHawk = (LuaI.GetMyHawk() ~= nil)
	local hasHawkTarget = (LuaI.GetTargetHawk() ~= nil)
...
function AccountAction(loginName, shadow, total)
	--print(string.format("Lua: AccountAction, <%s>, %d, %d",
	--	loginName, shadow, total), prefix, suffix)

	local r = math.random(0, 99)
	if r < 33 or shadow < 2 or total == 0 then	-- 33% to remain same account
		LuaI.Execute("login")
	else
		local prefix, suffix = string.match(loginName, "(%a+)(%d+)")
		if prefix and suffix then
			local i = (tonumber(suffix) % total)
			i = i + total * math.random(0, shadow - 1)
			local fmt = string.format("login %%s%%0%dd", string.len(suffix))
			local cmd = string.format(fmt, prefix, i)
			--print("Lua: ", cmd)
			LuaI.Execute(cmd)
		end
	end
...
function StartGuildWarBid()
    if _G.GuildWarStatus == GuildWar.SecondRoundEnd then
        local r = math.random(0, 199)
        if r < 1 then -- 0.5% of chance will trigger the guild war 
	            ...
	            if map:GetMap_id() ~= GuildWarMap[myguild:GetGuildWarGroup()] then
				    ...
				    if isLeader then
				        if ((timenow - _G.GuildWarStartTime) > (16 * minute)) then
					        ...
					        elseif r < 50 then
					        	local cmd = "move g" .. math.random(1000)
					        	LuaI.Execute(cmd)	
					        end
				        end
					end
...
function GuildAction()
	...
	if LuaI.CheckGuildMembership() == false then -- if still not belongs to any guild
		local guildrand = math.random(0, 199)
		if guildrand < 1 then -- 0.5% for create a new guild
			cmd = "cheat guild create" -- create your own guild
		elseif guildrand < 2 then --  0.5% for joining the existing guild
		...
		local r = math.random(0, 199)
		if r < 1 then -- 0.5% for handling the membership application
		...
		elseif r < 2 then -- 0.5% for kicking out member
			local totalMember = LuaI.GetTotalGuildMemberNumber() - 1
			if totalMember > 0 then
				cmd = "guild kickmember " .. math.random(0, totalMember)
			end
		elseif r < 3 then -- 0.5% for changing member position
			local totalMember = LuaI.GetTotalGuildMemberNumber() - 1    
			if totalMember > 0 then
				cmd = "guild changepost " .. math.random(0, totalMember) .. 
						" " .. math.random(1, 10)			  
			end
		elseif r < 4 then -- 0.5% for leaving your own guild
			cmd = "guild leave"
		elseif r < 5 then -- 0.5% for writing down your own guild aim
		...
		elseif r < 6 then -- 0.5% for writing down your own guild rule
			local lenghtRand = math.random(1, 20)
...
function ShopAction(map, character)
	...
	local r = math.random(0, 199)
	if r < 1 then  --%0.5 for buying in onlineShop
    	OpenShop()
	end
...
function QuestAction()
	...
	if tp then
		...
		if (r < 1) and (tp:IsOutstandingQuestTask() == false) then -- 1% for quest cancel
		    cmd = "quest cancel"
		    --print("QuestAction 2a: cancel quest")	
		elseif (r < 2) or (OneSubQuestRunning) then-- 1% for completing the quest while 100% if subquests is running
...
function FriendAction(map, character)
	...
	elseif r < 2 then	-- 0.5%: delete a friend
		cmd = "friend delete 0"
	elseif r < 3 then	-- 0.5%: move friend group
		cmd = "friend move_group 0 " .. math.random(2, 5) 	
	elseif r < 9 then	-- 3%: send message to a friend
...
function GiveAction(map, character)
	...
	if char2 then -- if a person is nearby, carry out the "give" command
		if r < 3 then -- 3%: give money
            ...
            cmd = "give " .. char2:GetChar_id() .. " money " .. money
    	elseif r < 5 then -- 2%: give partner
			local partnerGroup = LuaI.GetPartnerGroup()
			local count = partnerGroup:GetCount()
			if count > 0 then
				local index = math.random(count)
				cmd = "give " .. char2:GetChar_id() .. " partner " .. index
			end
		elseif r < 7 then -- 2%: give pet
			local petGroup = LuaI.GetPetGroup()
			local count = petGroup:GetCount()
			if count > 0 then
				local index = math.random(count)
				cmd = "give " .. char2:GetChar_id() .. " pet " .. index
			end
 	   	elseif r < 12 then -- 5%: give item
			local bag = LuaI.GetItemBag()
			local count = bag:GetCount(0)
			local bagIndex = 255
		
			if count > 0 then
				bagIndex = math.random(bag:GetCount(0))
				local item = bag:GetItem(bagIndex)
				if item == nil or item:IsBound() then
					bagIndex = character:GetValidItem()
				end
			end
			if bagIndex ~= 255 then
     	    	cmd = "give " .. char2:GetChar_id() .. "item " .. bagIndex
			end
    	end
	end
...
function MoveAction(map, character)
	...
	if character:GetAction() == LuaI.CMapActor_ST_MOVE then
		...
		-- 1%: try do PK battle with other character
		elseif r < 38 and character:GetEnablePK() then
			local char2 = map:GetNearCharacter(character:GetCurPosX(), 
...
function SkillAction(map, character)
	...
	--  0.5%: learn or upgrade new skill
	if r < 1 then
		local t = {1000, 1010, 1030, 1040, 1200, 1210, 1220, 1230}
	...
	--  0.5%: learn or remove one partner skill
	--elseif r < 3 then
	--	local partnerGroup = LuaI.GetPartnerGroup()
	--	local partner = nil
	--	local pi = 0
