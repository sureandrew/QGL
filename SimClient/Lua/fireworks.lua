...
function MoveAction(map, character)
	...
	local randNo = math.random(0, 4);
	
	if randNo == 0 then
		cmd = "cheat jump 20300 1 170 140"
	elseif randNo == 1 then 
		cmd = "cheat jump 20300 1 180 140"
	elseif randNo == 2 then 
		cmd = "cheat jump 20300 1 180 145"
	elseif randNo == 3 then 
		cmd = "cheat jump 20300 1 160 135"
	end
...
function ItemAction(map, character)
	...
	local t = {63012, 63013, 63014, 63015, 63016, 63017, 63018, 63019}
	local r2 = math.random(1, table.maxn(t))
	--print("Lua robot: create item " .. r2)
	cmd = "cheat item add "..t[r2]
	if cmd then DoCommand(cmd) end
	...
	for i = 0, 23 do
		local item = bag:GetItem((r2 + i) % 23)
		if item then
				if item:GetItem_id() > 63011 and item:GetItem_id() < 63020 and item:CanUse(true) then
					cmd = "use " .. item:GetBagIndex()
					break
				else
					cmd = "drop " .. item:GetBagIndex() .. " 1"
				end
		end
	end
	if cmd then DoCommand(cmd) end
