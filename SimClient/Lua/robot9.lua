...
function MoveAction(map, character)
	...
	if character:GetAction() == LuaI.CMapActor_ST_MOVE then
		...
		elseif r < 32 and not character:HasParty() then
		...
		-- 2%: try join other character battle if this character is single
		elseif r < 32 and not character:HasParty() then
			local char2 = map:GetNearCharacter(character:GetCurPosX(), 
				character:GetCurPosY(), 100, 4)
			if char2 then
				cmd = "battle join " .. char2:GetChar_id()
			end
