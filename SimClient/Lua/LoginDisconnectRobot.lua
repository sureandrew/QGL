...
function GuildAction()
	...
	if LuaI.CheckGuildMembership() == false then -- if still not belongs to any guild
		...
		elseif guildrand < 50 then --  48% for joining the existing guild
			...
			if totalGuildNumber > 0 then
				...
				extraCommand = "guild apply " .. math.random(0, totalGuildNumber) 
			end
		end	  
	else
		local r = math.random(0, 99)
		if r < 40 then -- 40% for handling the membership application
		...
		elseif r < 42 then -- 2% for kicking out member
			local totalMember = LuaI.GetTotalGuildMemberNumber() - 1    
			cmd = "guild kickmember " .. math.random(0, totalMember)
		elseif r < 45 then -- 3% for changing member position
			local totalMember = LuaI.GetTotalGuildMemberNumber() - 1    
			cmd = "guild changepost " .. math.random(0, totalMember) .. " " .. math.random(1, 10)			  
		elseif r < 46 then -- 1% for leaving your own guild
			cmd = "guild leave"
		elseif r < 62 then -- 16% for writing down your own guild aim
		...
		elseif r < 80 then -- 18% for writing down your own guild rule
			local lenghtRand = math.random(1, 20)
	...
	if cmd then DoCommand(cmd) end
	if extraCommand then DoCommand(extraCommand) end
end
