...
// : add the following macro, otherwise could not compile...
#pragma warning(push)
...
#pragma warning(disable: 4244 4345)
...
#define _BLOB_ENTRY_CODE(nOrdinal, IID, flags, dataOffset, lengthOffset, statusOffset) \
	...
	{ \
		...
		ATLTRY(pObject = SafeCreateObject(DBOBJECT)); \
		if (pObject == NULL) \
...
public:
	...
	{
		m_pBuffer = NULL;
		ATLTRY(m_pBuffer = SafeAllocate(BYTE, nSize));
		m_nSize = (m_pBuffer == NULL) ? 0 : nSize;
	}
	virtual ~CBookmark()
	{
		SafeDeallocate(m_pBuffer);
	}
...
public:
	...
	HRESULT ReleaseAccessors(IUnknown* pUnk) throw()
	{
		...
		if (m_nAccessors > 0)
		{
			...
			m_nAccessors = 0;
			SafeDeallocate(m_pAccessorInfo);
			m_pAccessorInfo = NULL;
		}
		return hr;
	}
...
// Implementation
	...
	HRESULT AllocateAccessorMemory(int nAccessors) throw()
	{
		...
		m_pAccessorInfo = NULL;
		ATLTRY(m_pAccessorInfo = SafeAllocate(_ATL_ACCESSOR_INFO, nAccessors));
...
#endif
		for (i=0; i<nColumns; i++)
			SafeDeleteObject(pBindings[i].pObject);
...
public:
	...
	HRESULT GetXMLColumnInfo( CSimpleStringW& strOutput ) throw()
	{
		...
		{
			ATLTRY(m_pXMLAccessor = SafeCreateObject(CXMLAccessor));
			if( m_pXMLAccessor == NULL )
				return E_OUTOFMEMORY;
			hr = m_pXMLAccessor->BindColumns( m_spRowset );
			if( FAILED(hr) )
			{
				SafeDeleteObject(m_pXMLAccessor);
				m_pXMLAccessor = NULL;
				return hr;
			}
		}
...
// Implementation
	HRESULT BindFinished() throw()
	{
		...
		if (m_phRow == NULL)
		{
			ATLTRY(m_phRow = SafeAllocate(HROW, m_nRows));
			if (m_phRow == NULL)
				return E_OUTOFMEMORY;
		}
...
public:
	...
	CVirtualBuffer(int nMaxElements)
	{
		...
		if(m_pBase == NULL)
		{
			_AtlRaiseException(STATUS_NO_MEMORY);
		}
...
// Implementation
	...
	HRESULT BindAccessor(IAccessor* pAccessor, ULONG nAccessor, DBLENGTH nSize) throw()
	{
		...
		else
		{
			...
			for( ULONG i = 0; i < nColumns; i++ )
				SafeDeleteObject(spBindings[i].pObject);
...
//	void FreeRecordMemory(int , IRowset* ) throw() {}
	...
	void Close() throw()
	{
		...
		SafeDeallocate(m_pfClientOwnedMemRef);
		m_pfClientOwnedMemRef = NULL;
...
// Implementation
	...
	HRESULT BindColumns(IUnknown* pUnk) throw()
	{
		...
		ATLTRY( pBinding = SafeAllocate(DBBINDING, m_nColumns) );
		if( pBinding == NULL )
			return E_OUTOFMEMORY;

		ATLASSERT(m_pfClientOwnedMemRef == NULL);
		ATLTRY(m_pfClientOwnedMemRef = SafeAllocate(bool, m_nColumns));
		if( m_pfClientOwnedMemRef == NULL )
		{
			SafeDeallocate(pBinding);
			return E_OUTOFMEMORY;
		}
		...
		for (i = 0; i < m_nColumns; i++)
		{
			...
			if (m_pColumnInfo[i].ulColumnSize > m_nBlobSize && m_pColumnInfo[i].wType != DBTYPE_IUNKNOWN )
			{
				...
				else // if( m_eBlobHandling == DBBLOBHANDLING_DEFAULT )
				{
					...
					{
						for( ULONG t = 0; t < i; t++ )
							SafeDeleteObject(pBinding[t].pObject);
						SafeDeallocate(pBinding);
		...
		m_pBuffer = NULL;
		ATLTRY(m_pBuffer = SafeAllocate(BYTE, nOffset));
		if (m_pBuffer == NULL)
		{
			for( ULONG t = 0; t < m_nColumns; t++ )
				SafeDeleteObject(pBinding[t].pObject);
...
public:
	...
	{
		SafeDeallocate(m_pParameterEntry);
		if (m_ppParamName != NULL)
		{
			CoTaskMemFree(*m_ppParamName);
			SafeDeallocate(m_ppParamName);
		}
		SafeDeleteObject(m_pParameterBuffer);
	};
...
// Implementation
				...
				void** ppParameterBuffer, bool fBindLength = false, bool fBindStatus = false ) throw()
	{
		...
		m_pParameterBuffer = NULL;
		ATLTRY(m_pParameterBuffer = SafeAllocate(BYTE, nOffset));
	...
	HRESULT AllocateParameterInfo(DB_UPARAMS nParamEntries) throw()
	{
		...
		m_pParameterEntry = NULL;
		ATLTRY(m_pParameterEntry = SafeAllocate(DBBINDING, nParamEntries));
		if (m_pParameterEntry == NULL)
			return E_OUTOFMEMORY;

		// Allocate memory to store the field names
		m_ppParamName = NULL;
		ATLTRY(m_ppParamName = SafeAllocate(OLECHAR*, nParamEntries));
		if (m_ppParamName == NULL)
		{
			SafeDeallocate(m_pParameterEntry);
			m_pParameterEntry = NULL;
...
public:
	...
	HRESULT CreateAccessor(int nBindEntries, void* pBuffer, DBLENGTH nBufferSize) throw()
	{
		...
		// Allocate memory for the bind structures
		ATLTRY(m_pEntry = SafeAllocate(DBBINDING, nBindEntries));
		if (m_pEntry == NULL)
			return E_OUTOFMEMORY;
		else
			return S_OK;
	...
	HRESULT CreateParameterAccessor(int nBindEntries, void* pBuffer, DBLENGTH nBufferSize) throw()
	{
		...
		// Allocate memory for the bind structures
		ATLTRY(m_pParameterEntry = SafeAllocate(DBBINDING, nBindEntries));
		if (m_pParameterEntry == NULL)
			return E_OUTOFMEMORY;
		else
			return S_OK;
...
public:
	...
	HRESULT SetParameterBuffer(BYTE* pBuffer) throw()
	{
		...
		ATLTRY(m_pParamInfo = SafeCreateObject(_ATL_COLUMN_PARAM_INFO));
...
// Implementation
	...
	HRESULT AddAccessorInfo() throw()
	{
		...
		ATLTRY( pAccessorInfo = SafeAllocate(_ATL_ACCESSOR_INFO, m_nAccessors + 1); )
